...
The handler interrogates the database for the next available free slots of the current day: if successful it generates a ticket with its own QR code (using the QRCodeGenerator component) and sends it to the App; it sends an error otherwise.\\
...
\begin{figure}[H]
    ...
    \end{center}
\end{figure}
The previous diagram describes the process of scanning a customer's QR code.\\
...
This last one interrogates the database to validate the the code: if the query returns a value it means the code is valid, otherwise it sends an error.\\
If the validation is successful, the QRCodeValidation handler reports it to the ClientManager and to the App, where a message status it's displayed.
